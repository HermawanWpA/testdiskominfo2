...
<div class="card shadow align-items-center" style="width: 30rem;">
    <div class="card-body">
        <form action="{{ route('update-wifi', $item->id) }}" method="post">
            @method('PUT')
         @csrf
        <div class="row">
          <div class="col">
            {{-- <input type="number" name="id_biodata" hidden> --}}
                <div class="form-group mb-3">
                    <label for="Nama Wifi">Nama Wifi</label>
                    <input type="text" class="form-control" name="nama_wifi" placeholder="Nama Wifi" value="{{ $item->nama_wifi }}">
                </div>
            </div>
                <div class="col">
                    <div class="form-group mb-3">
             ...
             <div class="col-md-12">
                   <div class="form-group">
                    ...
                    <select name="id_lokasi" class="form-control">
                            {{-- <option value="{{ $item->id }}"selected>{{ $item->biodata->nama_pegawai }}</option> --}}
                        @foreach ($data as $item)
                            <option value="{{ $item->id }}">{{ $item->lokasi1 }}</option>
                        @endforeach
                    </select>
                   </div>
               </div>
